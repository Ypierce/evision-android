...
public class CircularProgressView extends View {
	...
	private void initAttributes(AttributeSet attrs, int defStyle) {
		...
		int accentColor = getContext().getResources().getIdentifier("colorAccent", "attr",
...
//		 If using native accentColor (SDK >21)
			...
			TypedArray t = getContext().obtainStyledAttributes(new int[] { android.R.attr.color });
	...
	/**
	 * Sets whether this CircularProgressView is indeterminate or not. It will
	 * reset the animation if the mode has changed.
	 * 
	 ...
	 * 
	 * @param thickness
	 *            the thickness of the progress bar arc
	 */
	...
	/**
	 * Starts the progress bar animation. (This is an alias of resetAnimation()
	...
	private AnimatorSet createIndeterminateAnimator(float step) {
		...
		ValueAnimator rotateAnimator2 = ValueAnimator.ofFloat((step + .5f) * 720f / animSteps,
				(step + 1) * 720f / animSteps);
