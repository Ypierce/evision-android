...
public class PullToRefreshLayout extends RelativeLayout {
	...
	private void initView(Context context, AttributeSet attrs, int defStyle) {
		...
		defaultRefreshView = inflater.inflate(R.layout.refresh_head, this, false); // 上拉头
		refreshView = defaultRefreshView;
		defaultLoadmoreView = inflater.inflate(R.layout.load_more, this, false);
		loadmoreView = defaultLoadmoreView; // 下拉头
	...
	public void refreshFinish(int refreshResult) {
		if (null != mOnRefreshProcessListener) {
			mOnRefreshProcessListener.onFinish(refreshView, OnPullProcessListener.REFRESH);
		}
	...
	public void loadmoreFinish(int refreshResult) {
		if (null != mOnLoadmoreProcessListener) {
			mOnLoadmoreProcessListener.onFinish(loadmoreView, OnPullProcessListener.LOADMORE);
		}
	...
	private void changeState(int to) {
		...
		case INIT:
			if (null != mOnRefreshProcessListener) {
				mOnRefreshProcessListener.onPrepare(refreshView, OnPullProcessListener.REFRESH);
			}
			if (null != mOnLoadmoreProcessListener) {
				mOnLoadmoreProcessListener.onPrepare(loadmoreView, OnPullProcessListener.LOADMORE);
			}
			// 下拉布局初始状态
			if (null == customRefreshView) {
		...
		case RELEASE_TO_REFRESH:
			if (null != mOnRefreshProcessListener) {
				mOnRefreshProcessListener.onStart(refreshView, OnPullProcessListener.REFRESH);
			}
			// 释放刷新状态
			if (null == customRefreshView) {
		...
		case REFRESHING:
			if (null != mOnRefreshProcessListener) {
				mOnRefreshProcessListener.onHandling(refreshView, OnPullProcessListener.REFRESH);
			}
			// 正在刷新状态
			if (null == customRefreshView) {
		...
		case RELEASE_TO_LOAD:
			if (null != mOnLoadmoreProcessListener) {
				mOnLoadmoreProcessListener.onStart(loadmoreView, OnPullProcessListener.LOADMORE);
			}
			// 释放加载状态
			if (null == customLoadmoreView) {
		...
		case LOADING:
			// 正在加载状态
			if (null != mOnLoadmoreProcessListener) {
				mOnLoadmoreProcessListener.onHandling(loadmoreView, OnPullProcessListener.LOADMORE);
			}
			if (null == customLoadmoreView) {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			float deleaX = ev.getX() - lastX;
			
			//如果横向下滑的值乘以2大于竖向下滑的值，我们认为用户行为不是下滑，直接交给父类处理
			if (2 * Math.abs(deleaX) > Math.abs(deltaY)) {
				super.dispatchTouchEvent(ev);
                return true;
			}
			
			if (null != mOnRefreshProcessListener) {
				mOnRefreshProcessListener.onPull(refreshView, deltaY, OnPullProcessListener.REFRESH);
			}
			if (null != mOnLoadmoreProcessListener) {
				mOnLoadmoreProcessListener.onPull(loadmoreView, pullUpY, OnPullProcessListener.LOADMORE);
			}
			if (mEvents == 0) {
				if (pullDownY > 0 || (((Pullable) pullableView).canPullDown() && mCanPullDown && mPullDownEnable
						&& state != LOADING)) {
					// 可以下拉，正在加载时不能下拉
					// 对实际滑动距离做缩小，造成用力拉的感觉
				...
				} else if (pullUpY < 0 || (((Pullable) pullableView).canPullUp() && mCanPullUp && mPullUpEnable
						&& state != REFRESHING)) {
					// 可以上拉，正在刷新时不能上拉
	...
	private void initView() {
		...
		if (null == customLoadmoreView) {
			...
			loadStateTextView = (TextView) defaultLoadmoreView.findViewById(R.id.loadstate_tv);
			loadingView = defaultLoadmoreView.findViewById(R.id.loading_icon);
			loadStateImageView = defaultLoadmoreView.findViewById(R.id.loadstate_iv);
		}
	}
	...
	class UpdateHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (null != layout) {
				...
				layout.mMoveSpeed = (float) (8 + 5 * Math
						.tan(Math.PI / 2 / layout.getMeasuredHeight() * (layout.pullDownY + Math.abs(layout.pullUpY))));
				if (!layout.isTouch) {
					// 正在刷新，且没有往上推的话则悬停，显示"正在刷新..."
					if (layout.state == REFRESHING && layout.pullDownY <= layout.refreshDist && mPullDownEnable) {
						layout.pullDownY = layout.refreshDist;
						layout.timer.cancel();
					} else if (layout.state == LOADING && -layout.pullUpY <= layout.loadmoreDist && mPullUpEnable) {
	...
	public class GifOnPullProcessListener implements OnPullProcessListener {
		...
		public void onPull(View v, float pullDistance, int which) {
			...
			RelativeLayout headView = (RelativeLayout) v.findViewById(R.id.head_view);
